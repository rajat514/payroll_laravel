...
class SalariesSheetExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $LIC = $netSalaries->sum(fn($item) => $item->deduction->lic ?? 0);
                $totalPay = $netSalaries->sum(
                    fn($item) => (($item->paySlip->total_pay ?? 0) +
                        ($item->deduction->lic ?? 0) +
                        ($item->deduction->credit_society ?? 0) -
                        ($item->deduction->total_deductions ?? 0))
                );
                $totalCreditSociety = $netSalaries->sum(fn($item) => $item->deduction->credit_society ?? 0);
                ...
                $sheet->setCellValue("F{$row}", $totalPay);  // Total Deductions
                $sheet->setCellValue("G{$row}", $LIC);     // Basic Pay
                $sheet->setCellValue("H{$row}", $totalCreditSociety);     // Basic Pay
                $sheet->setCellValue("I{$row}", $totalNetAmount);
    ...
    public function map($netSalary): array
    {
        ...
        $net_pay = ($netSalary->net_amount ?? 0)
            + (($deduction->lic ?? 0) + ($deduction->credit_society ?? 0));
        ...
        return [
            ...
            $employee->name,
            $net_pay,
            $this->safeValue($deduction->lic ?? 0),
            $this->safeValue($deduction->credit_society ?? 0),
