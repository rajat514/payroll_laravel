...
class GISSheet implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    public function collection()
    {
        ...
        return $query->whereHas(
            'employeeRelation',
            fn($qn) => $qn->where('gis_eligibility', 1)
        )->with([
            // 'employee.employeePayStructure.PayMatrixCell.payMatrixLevel:id,name',
            // 'employee:id,employee_code,prefix,first_name,middle_name,last_name,increment_month,pension_number,gis_no',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $totalGIS = $netSalaries->sum(fn($item) => $item->deduction->gis ?? 0);
                ...
                $sheet->setCellValue("I{$row}", $totalGIS);
