...
class ProfessionalTaxSheet implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    protected $filters;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $professional_tax = $netSalaries->sum(fn($item) => $item->deduction->professional_tax ?? 0);
                ...
                $sheet->setCellValue("G{$row}", $professional_tax);  // Total Deductions
                $sheet->setCellValue("F{$row}", $totalNetAmount);   // Net Amount
