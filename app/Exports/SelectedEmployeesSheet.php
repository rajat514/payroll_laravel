...
class SelectedEmployeesSheet implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    private function prepareDynamicColumns(): void
    {
        ...
        self::$netSalaries = $query->with([
            ...
            // 'employeeRelation.employeePayStructure.PayMatrixCell.payMatrixLevel:id,name',
    ...
    public function map($netSalary): array
    {
        ...
        // dd($netSalary->employee->employee_pay_structure);
        $bank = $netSalary->employee_bank ?? (object) [];
        $paySlip = optional($netSalary->paySlip);
        $deduction = optional($netSalary->deduction);

        // $pay_level = optional(optional(optional($netSalary->employee->employee_pay_structure)->pay_matrix_cell)->pay_matrix_level) ?? null;
        $pay_level = $netSalary->employee->employee_pay_structure->pay_matrix_cell->pay_matrix_level ?? null;

        $employee_increment_month = Carbon::create()->month($employee->increment_month)->format('F');
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $IncomeTaxCol = array_search('Income Tax', $this->headings()) + 1;
                ...
                $sheet->setCellValueByColumnAndRow($IncomeTaxCol, $row, $totalIncomeTax);
